...
" History:      {{{1
" Version 5.0:  * Compatible with Srinath Avadhanula's imaps.vim
...
" Make sure imaps.vim is loaded before this plugin if it is installed.
if !exists("*IMAP")
...
if exists("s:loaded_common_brackets") 
      \ && !exists('g:force_reload_common_brackets')
  let &cpo = s:cpo_save
  finish 
endif
let s:loaded_common_brackets = 1
...
function! Brkt_lt() " {{{
  if exists('b:cb_ltFn')
    ...
    return "\<C-R>=BuildMapSeq(" . b:cb_ltFn . ")\<CR>"
    " return "\<C-R>=BuildMapSeq(escape(" . b:cb_ltFn . ",'\\'))\<CR>"
    return BuildMapSeq({b:cb_ltFn}) 
  else
    if exists('*IMAP')
...
function! Brkt_gt() " {{{
  if exists('b:cb_gtFn')        | return "\<C-R>=BuildMapSeq(" . b:cb_gtFn . ")\<CR>"
...
function! Brkt_Dquote() " {{{
  if b:cb_Dquotes == 2
    ...
    if exists('b:cb_DqFn')
      ...
      return "\<C-R>=BuildMapSeq(escape(" . b:cb_DqFn . ", '\'))\<CR>"
